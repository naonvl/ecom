...
class CustomFormBuilderController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        $id = sanitize_html($request->id);
        $title = sanitize_html($request->title);
        $email = sanitize_html($request->email);
        $button_title = sanitize_html($request->button_title);
        ...
        foreach ($request->field_name as $fname) {
            $all_fields_name[] = strtolower(Str::slug(sanitize_html($fname)));
        }
        ...
        FormBuilder::findOrFail($id)->update([
            ...
            'button_text' => sanitize_html($button_title),
            'success_message' => sanitize_html($request->success_message),
    ...
    public function store(Request $request)
    {
        ...
        FormBuilder::create([
            'title' => sanitize_html($request->title),
            'email' => sanitize_html($request->email),
            'button_text' => sanitize_html($request->button_title),
            'success_message' => sanitize_html($request->success_message),
        ]);
