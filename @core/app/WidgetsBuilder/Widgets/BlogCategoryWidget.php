...
class BlogCategoryWidget extends WidgetBase
{
    ...
    public function frontend_render()
    {
        ...
        $post_items = sanitize_html($widget_saved_values['post_items']) ?? '';
        ...
        foreach ($blog_posts as $post) {
            $output .= '<li><a href="' . route('frontend.blog.category', ['id' => $post->id,'any' => Str::slug($post->name)]) . '"><i class="las la-angle-right"></i> ' . purify_html($post->name) . '</a></li>';
        }
        $output .= '</ul>';
