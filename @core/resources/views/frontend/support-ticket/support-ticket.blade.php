                ...
                <div class="col-lg-8">
                    <div class="support-ticket-wrapper">
                        @if (auth()->guard('web')->check())
                            <h3 class="title">
                                {{ get_static_option('support_ticket_' . $user_select_lang_slug . '_form_title') }}</h3>
                            <x-msg.flash />
                            <x-msg.error />
                            <form action="{{ route('frontend.support.ticket.store') }}" method="post"
                                ...
                                <div class="btn-wrapper text-center">
                                    <button
                                        type="submit">{{ get_static_option('support_ticket_' . $user_select_lang_slug . '_button_text') }}</button>
                                </div>
                            </form>
                        @else
                            @include('frontend.partials.ajax-login-form',['title' =>
                            get_static_option('support_ticket_'.$user_select_lang_slug.'_login_notice')])
                        @endif
                    </div>
                </div>
            </div>
